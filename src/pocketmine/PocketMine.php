...
namespace pocketmine {
	...
	const CODENAME = "[Katana/Slice]";
	const MINECRAFT_VERSION = "1.5.2";
	const MINECRAFT_VERSION_NETWORK = "1.5.2";
	...
	if(!\extension_loaded("pthreads")){
		echo "[ERRO] Unable to find the pthreads extension." . \PHP_EOL;
	...
	if(\php_sapi_name() !== "cli"){
		$logger->critical("You must run SlicePocketMine using the CLI.");
		++$errors;
	}
	...
	if(\Phar::running(\true) === ""){
		$logger->warning("Non-packaged SlicePocketMine installation detected, do not use on production.");
	}
